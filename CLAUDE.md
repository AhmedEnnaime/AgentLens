...
- **One bounded context = one package; no mixed-purpose packages.** Files that serve different businesses never share a package. A file moves out of its package the moment it serves another context's purpose (e.g. `internal/domain/time.go` — an epoch-ms translation helper — is ingest-layer knowledge, not domain logic; its relocation is tracked in issue #9). Within a context, files are cohesive parts of one model (e.g. `internal/domain` files are one canonical language, layered value-objects → vocabularies → entities — not candidates for sub-packages). Sub-package splits within a context are proposed via ADR only when a context genuinely outgrows one package — never as structural ceremony. The Reviewer checks this on every PR.
